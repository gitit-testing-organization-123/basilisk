...
/** This macro walks all cells of the domain, excluding nonactive ghosts */

#if dimension == 1
macro2 foreach_cell(ivec d = Dimensions) {
  {
    ...
    for (int ox = 0; ox < d.x; ox++) {
      for (int oy = 0; oy < d.y; oy++) {
        for (int oz = 0; oz < d.z; oz++) {
          Point root = {GHOSTS + ox, GHOSTS + oy, GHOSTS + oz, 0};
          foreach_cell_root(root)
          // clang-format off
            {...}
          // clang-format on
...
// #if dimension >= 2
//       for (root.j = GHOSTS*Period.y; root.j <= GHOSTS*(2 - Period.y);
//       root.j++)
// #endif
// #if dimension >= 3
// 	for (root.k = GHOSTS*Period.z; root.k <= GHOSTS*(2 - Period.z);
// root.k++) #endif 	  foreach_cell_root (root)
// 	    {...}
//   }
// }

/** This macro walks all cells of the domain, including nonactive ghosts */
macro2 foreach_cell_all(ivec d = Dimensions) {
  {
    Point root = {0};
    for (root.i = GHOSTS * Period.x; root.i < d.x + GHOSTS * (2 - Period.x);
         root.i++)
#if dimension >= 2
      for (root.j = GHOSTS * Period.y; root.j < d.y + GHOSTS * (2 - Period.y);
           root.j++)
#endif
#if dimension >= 3
        for (root.k = GHOSTS * Period.z; root.k < d.z + GHOSTS * (2 - Period.z);
             root.k++)
#endif
          foreach_cell_root(root)
          // clang-format off
...
#if dimension == 1
macro2 foreach_cell_post(bool condition, ivec d = Dimensions) {
  {
    for (int ox = 0; ox < d.x; ox++) {
      Point root = {GHOSTS + ox, 0};
      foreach_cell_post_root(condition, root)
...
macro2 foreach_cell_post_all(bool condition, ivec d = Dimensions) {
  {
    Point root = {0};
    for (root.i = 0; root.i < d.x + 2 * GHOSTS; root.i++)
#if dimension >= 2
      for (root.j = 0; root.j < d.y + 2 * GHOSTS; root.j++)
#endif
#if dimension >= 3
        for (root.k = 0; root.k < d.z + 2 * GHOSTS; root.k++)
#endif
          foreach_cell_post_root(condition, root)
          // clang-format off
            {...}
