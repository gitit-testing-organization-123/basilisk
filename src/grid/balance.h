...
macro2 foreach_tree(Array *t, size_t size, scalar *list,
...
static void receive_tree(int from, scalar newpid, FILE *fp) {
  Array a;
  mpi_recv_check(&a.len, 1, MPI_LONG, from, MOVED_TAG(), MPI_COMM_WORLD,
                 MPI_STATUS_IGNORE, "receive_tree (len)");
  ...
  if (a.len > 0) {
    ...
    mpi_recv_check(a.p, a.len, MPI_BYTE, from, MOVED_TAG(), MPI_COMM_WORLD,
                   MPI_STATUS_IGNORE, "receive_tree (p)");
    ...
    foreach_tree(&a, sizeof(Cell) + datasize, NULL) {
      ...
      if (fp) {
        fprintf(fp, "%g %g %g %d %d %d %d %d %d recv\n", x, y, z,
                NEWPID()->pid - 1, cell.pid, c->flags & leaf, cell.flags & leaf,
                from, NEWPID()->leaf);
        }
    }
...
trace bool balance() {
  ...
  if (DEBUGCOND) {
    ...
    foreach_cell() {
      fprintf(fp, "%g %g %g %d %d %d %d\n", x, y, z, cell.pid,
              NEWPID()->pid - 1, NEWPID()->leaf, is_leaf(cell));
      if (NEWPID()->leaf)
        assert(is_leaf(cell));
    }
    fclose(fp);
      ...
      if (is_newpid()) {
        if (fp) {
          fprintf(fp, "%g %g %g %d %d %d %d %d new\n", x, y, z,
                  NEWPID()->pid - 1, cell.pid, is_leaf(cell), cell.neighbors,
                  NEWPID()->leaf);
        }
  ...
  if (tree->dirty || pid_changed) {
    ...
    /* Update parent pids and active flags after migrated cells have been
       received.  This must run for every non-leaf, including local parents:
       in forests, level-0 cell.pid is used by z_indexing() to decide which rank
       contributes each root's subtree size.  A stale root pid makes the next
       z_indexing() count old root sizes even though balance() sees fewer
       locally-owned leaves.

       The shorter version below only repaired non-local parents.  That is not
       enough for forests because a root can stay local while its children have
       moved.  Keeping it here documents the tempting but incomplete form.
    ...
    */
    ...
    foreach_cell_post(!is_leaf(cell)) {
      if (!is_leaf(cell)) {
        if (cell.pid >= 0) {
          int cpid = is_newpid() ? NEWPID()->pid - 1 : child(0).pid;
          if (cell.pid != cpid) {
            cell.pid = cpid;
            pid_changed = true;
          }
